...
namespace DXApplication1
{
    ...
    public sealed class DatabaseHelper
    {
        ...
        public bool ToggleFavorite(int movieId, bool isFavorite)
        {
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (ToggleFavorite): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        public DataTable GetFavoriteMovies()
        {
            ...
            {
                using var conn = new NpgsqlConnection(_connectionString);
                using var cmd = new NpgsqlCommand("SELECT id, movie_name, genre, COALESCE(rating,0.0) AS rating, COALESCE(is_favorite,false) AS is_favorite FROM movies WHERE is_favorite = true ORDER BY rating DESC", conn);
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (GetFavoriteMovies): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return dt;
        }
        ...
        public DataTable GetMoviesByGenre(string genre)
        {
            ...
            try
            {
                ...
                using var cmd = new NpgsqlCommand("SELECT id, movie_name, genre, COALESCE(rating,0.0) AS rating, COALESCE(is_favorite,false) AS is_favorite FROM movies WHERE genre ILIKE @g ORDER BY rating DESC", conn);
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (GetMoviesByGenre): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return dt;
        }
        ...
        public (int id, string name, double rating)? GetRandomMovieByGenre(string genre, int? excludeId = null)
        {
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (GetRandomMovieByGenre): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
        }
        ...
        public DataTable GetMovies()
        {
            ...
            {
                XtraMessageBox.Show($"Veritabaný baðlantý hatasý:\n{ex}", "Veritabaný Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return dt;
        }
        ...
        public System.Collections.Generic.List<(int id, string name, int? year)> GetMoviesWithZeroRating()
        {
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (listeleme): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            return list;
        }
        ...
        public bool UpdateMovieRating(int id, decimal rating)
        {
            ...
            {
                XtraMessageBox.Show($"Veritabaný hatasý (güncelleme): {ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        public int UpdateRatingsFromOmdb()
        {
            ...
            {
                XtraMessageBox.Show("OMDb API anahtarý App.config içinde ayarlý deðil.", "OMDb Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return -1;
            }
            ...
            {
                XtraMessageBox.Show($"OMDb güncelleme hatasý:\n{ex}", "OMDb Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
        }
        ...
        private string TranslateMovieName(string? name)
        {
            ...
            var map = new System.Collections.Generic.Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "The Godfather", "Baba" },
                { "The Dark Knight", "Kara Þövalye" },
                { "Pulp Fiction", "Ucuz Roman" },
                { "Schindler's List", "Schindler'in Listesi" },
                { "Fight Club", "Dövüþ Kulübü" },
                { "Forrest Gump", "Forrest Gump" },
                { "Inception", "Baþlangýç" },
                { "The Matrix", "Matrix" },
                { "Goodfellas", "Sýký Dostlar" },
                { "The Lion King", "Aslan Kral" },
                { "Back to the Future", "Geleceðe Dönüþ" },
                { "The Lord of the Rings: The Return of the King", "Yüzüklerin Efendisi: Kralýn Dönüþü" },
                { "The Lord of the Rings: The Fellowship of the Ring", "Yüzüklerin Efendisi: Yüzük Kardeþliði" },
                { "The Lord of the Rings: The Two Towers", "Yüzüklerin Efendisi: Ýki Kule" }
            };
