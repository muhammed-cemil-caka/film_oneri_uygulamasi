...
namespace DXApplication1
{
    internal class OmdbClient
    {
        ...
        // (plot fetching removed - not used anymore)
        ...
        // Search titles (s=) and return simple list of (Title, imdbID, Year)
        public async Task<System.Collections.Generic.List<(string title, string imdbId, int? year)>> SearchTitlesAsync(string query, int page = 1)
        {
            var list = new System.Collections.Generic.List<(string title, string imdbId, int? year)>();
            var q = Uri.EscapeDataString(query);
            var url = $"http://www.omdbapi.com/?s={q}&apikey={_apiKey}&page={page}";
            var res = await _http.GetAsync(url);
            if (!res.IsSuccessStatusCode) return list;
            using var stream = await res.Content.ReadAsStreamAsync();
            var doc = await JsonDocument.ParseAsync(stream);
            if (!doc.RootElement.TryGetProperty("Search", out var searchEl) || searchEl.ValueKind != JsonValueKind.Array) return list;
            foreach (var item in searchEl.EnumerateArray())
            {
                if (!item.TryGetProperty("Title", out var t) || !item.TryGetProperty("imdbID", out var id)) continue;
                string title = t.GetString() ?? string.Empty;
                string imdbId = id.GetString() ?? string.Empty;
                int? y = null;
                if (item.TryGetProperty("Year", out var ye))
                {
                    var ys = ye.GetString();
                    var m = System.Text.RegularExpressions.Regex.Match(ys ?? "", "(\\d{4})");
                    if (m.Success && int.TryParse(m.Groups[1].Value, out var yv)) y = yv;
                }
                list.Add((title, imdbId, y));
            }
            return list;
        }

        // Get details by imdbID
        public async Task<(double? rating, int? year, string? genre)> GetMovieDetailsByImdbIdAsync(string imdbId)
        {
            if (string.IsNullOrWhiteSpace(imdbId)) return (null, null, null);
            var url = $"http://www.omdbapi.com/?i={Uri.EscapeDataString(imdbId)}&apikey={_apiKey}";
            var res = await _http.GetAsync(url);
