            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (opt != null)
                        {
                            // try several common property names for corner radius
                            var tryNames = new string[] { "ItemCornerRadius", "CornerRadius", "ItemRadius", "ItemCornerRadiusPx", "Radius" };
                            foreach (var n in tryNames)
                            {
                                try
                                {
                                    var p = opt.GetType().GetProperty(n);
                                    if (p != null && p.CanWrite)
                                    {
                                        try { p.SetValue(opt, 12); break; } catch { }
                                    }
                                }
                                catch { }
                            }

                            // try to enable built-in shadow options if available
                            var shadowNames = new string[] { "ShowItemShadow", "EnableItemShadow", "ItemShadow", "ItemShadows", "UseShadows" };
                            foreach (var n in shadowNames)
                            {
                                try
                                {
                                    var p = opt.GetType().GetProperty(n);
                                    ...
                                    {
                                        try { p.SetValue(opt, true); } catch { }
                                    }
                                }
                                catch { }
                            }

                            // try setting a shadow color if available
                            try
                            {
                                var sc = opt.GetType().GetProperty("ItemShadowColor"); if (sc != null && sc.CanWrite) try { sc.SetValue(opt, Color.FromArgb(40, 0, 0, 0)); } catch { }
                            }
                            catch { }
                        }
                    }
                ...
                // Highlight movies by IMDb rating: green / yellow / red
                tile.ItemCustomize += (s, e) =>
                {
                    try
                    {
                        ...
                        if (drv != null)
                        {
                            ...
                            // apply color bands consistent with GridView row styling
                            if (rv >= 8.5)
                            {
                                // pastel green
                                e.Item.Appearance.BackColor = Color.FromArgb(235, 248, 236);
                                e.Item.Appearance.Options.UseBackColor = true;
                            }
                            else if (rv >= 7.0)
                            {
                                // pastel yellow
                                e.Item.Appearance.BackColor = Color.FromArgb(255, 252, 230);
                                e.Item.Appearance.Options.UseBackColor = true;
                            }
                            else
                            {
                                // pastel red
                                e.Item.Appearance.BackColor = Color.FromArgb(255, 241, 243);
                                e.Item.Appearance.Options.UseBackColor = true;
                            }
                            ...
                            try
                            {
                                if (e.RowHandle == _hoveredTileRowHandle)
                                {
                                    ...
                                    e.Item.Appearance.Options.UseBorderColor = true;
                                    // subtle brighten on hover without losing band color
                                    try { e.Item.Appearance.BackColor = ControlPaint.Light(e.Item.Appearance.BackColor); } catch { }
                                    e.Item.Appearance.Options.UseBackColor = true;
                                }
                            }
                            catch { }
